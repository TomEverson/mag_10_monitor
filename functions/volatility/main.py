...
from shared.models import VolatilitySpike

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

_GCS_BUCKET = os.environ["GCS_BUCKET_RAW"]
_TABLE = "{}.{}.volatility_spikes".format(
...
def handle(request):
    ...
    # 2. Validate
    try:
        event = VolatilitySpike.model_validate(payload)
    except ValidationError as exc:
        logger.error("Validation failed for volatility_spike: %s", exc)
        return f"Bad Request: {exc}", 400

    # 3. Archive raw event to GCS
    detected_dt = datetime.fromisoformat(event.detected_at.replace("Z", "+00:00"))
    gcs_path = (
        f"volatility_spike/{detected_dt.year:04d}/{detected_dt.month:02d}/"
